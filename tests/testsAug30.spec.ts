...
test.describe('Checkboxes and Radio Buttons Tests', () => {
  ...
  test('Check checkboxes 1 by 1', async ({ page }) => {
    ...
    // MAIN OBJECTIVE IS TO VALIDATE ALL 7 APPLICABLE CHECKBOXES ARE CHECKED (1)
    await verifyAll7ApplicableCheckboxesAreChecked(page);
  });

  test('Check checkboxes all at once PARALLEL', async ({ page }) => {
    await page.goto(url1);
    await page.waitForTimeout(5000);
    ...
    */
    ...
    if(evaluateBeforeClicking) {
      // Mouse clicks DO not overlap: OK
      await Promise.all([
      ...
      // Could lead to mouse overlap: CAUSE ERRORS, SYNTAX STILL VALID FOR OTHER ACITONS SUCH AS checking states or something else (but avoid doing clicks like this)
      await Promise.all([
        // all clicks are fighting for the mouse control
  ...
  async function verifyAll7ApplicableCheckboxesAreChecked(page: Page): Promise<void> {
    await expect(page.locator(checkAlone)).toBeChecked();
